...
# Load data
bat_data <- read.csv("C:Ahuacapan_capture_data.csv", stringsAsFactors = FALSE)
rfid_folder <- "RFID/Ahuacapan/"
...
cohort_transfer_summary("AF", c("AF", "AM"))
cohort_transfer_summary("AM", c("AF", "AM"))
...
p1 <- ggplot(hair_pct, aes(x = Cohort, y = Percent, fill = Category)) +
  ...
  #   legend.spacing.y = unit(0, "lines")  # adds vertical space between legend items
  # ))
...
################################################################################
#### RFID Detected per night - Ahuacapan (Figure 5G) ###########################
################################################################################

#Filter out nights with incomplete RFID data
night_summary <- night_summary |> 
  dplyr::filter(!Night %in% as.Date(c("2025-06-03", "2025-06-18")))

fig5G <- ggplot(night_summary, aes(x = Night, y = UniqueTagCount)) +
  geom_line(color = "#1D91C0", size = 1) +
  geom_point(color = "#0C2C84", size = 2) +
  # Shaded regions
  geom_rect(aes(xmin = as.Date("2025-06-10") + 0.2, xmax = as.Date("2025-06-11") - 0.2,
                ymin = -Inf, ymax = Inf),
            fill = "gray", alpha = 0.05, inherit.aes = FALSE) +
  geom_rect(aes(xmin = as.Date("2025-06-15") + 0.2, xmax = as.Date("2025-06-16") - 0.2,
                ymin = -Inf, ymax = Inf),
            fill = "gray", alpha = 0.05, inherit.aes = FALSE) +
  
  # Labels for shaded areas
  annotate("text", x = as.Date("2025-06-10") - 1.4, y = 32,
           label = "Capture 1", hjust = 0, size = 5.5, fontface = "bold") +
  annotate("text", x = as.Date("2025-06-15") - 1.4, y = 32,
...
################################################################################
#### Plot of captures by cohort/date (Figure 5E) ###############################
################################################################################
...
# Plot
fig5E <- ggplot(plot_df, aes(x = factor(capture_date), y = Count, fill = Cohort)) +
  ...
  scale_y_continuous(limits = c(0, 25), expand = c(0, 0)) + #105 - Jirosto; 25 - Ahuacapan 
